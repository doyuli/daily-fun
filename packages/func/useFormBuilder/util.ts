import { ElInput, ElSelect, ElOption, ElDatePicker, ElTimePicker, ElSwitch, ElCheckboxGroup, ElCheckbox, ElRadio, ElRadioGroup } from 'element-plus'
...
function transformSubOption(comp: Component, subComp: Component): Component {
    return (props, { attrs }) => {
        ...
        return h(
            ...
            {
                default: () => options?.map((v: Record<string, any>) =>
                    h(subComp, { ...v })
                )
            }
        )
